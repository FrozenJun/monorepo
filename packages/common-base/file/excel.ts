...
export class Excel {
   ...
   */
  public async getWorkSheet(i = 0): Promise<ExcelJS.Worksheet> {
  ...
  ): Promise<Record<(typeof header)[number], any>[]> {
  ...
  private setCells(
    ...
    this.worksheet?.eachRow({ includeEmpty: true }, (row, rowNumber) => {
      ...
      row.font = data[rowNumber - 2]?.fontStyle
    })
    // 获取每一列数据，再依次对齐
