...
function polylineMeasure(L) {
  ...
  L.Control.PolylineMeasure = L.Control.extend({
    ...
    options: {
      ...
      tooltipTextResume:
        '<br>Press ' + (isMacOS ? '⌘' : 'CTRL-key') + ' and click to <b>resume line</b>',
    ...
    _drawArrow: function (arcLine) {
      ...
      var newArrowMarker = L.marker(center, { icon: iconArrow, zIndexOffset: -50 }).addTo(
        this._layerPaint
      ) // zIndexOffset to draw arrows below tooltips
      if (!this._currentLine) {
      ...
      this._currentLine = {
        ...
        handleMarkers: function (latlng) {
          ...
          newCircleMarker.bindTooltip(
            ...
            { direction: 'top', opacity: 0.7, className: 'polyline-measure-popupTooltip' }
          )
    ...
    _resumePolylinePath: function (e) {
      if (e.originalEvent.ctrlKey === true || e.originalEvent.metaKey === true) {
