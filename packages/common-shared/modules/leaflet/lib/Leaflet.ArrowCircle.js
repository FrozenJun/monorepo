...
export default function (options) {
  L.DivIcon.ArrowCircle = L.DivIcon.extend({
    ...
    initialize: function (options) {
      ...
      this.iconAnchor = L.point(Number(options.size) / 2, Number(options.size) / 2)
    ...
    _createArrow: function () {
      ...
      return `<path d="${pathDescription}" style="fill: ${this.options.color}; opacity: ${this.options.opacity};"/>`
    },
    ...
    _createSVG: function () {
      ...
      let style = 'width:' + this.options.size + 'px; height:' + this.options.size + 'px;'
  ...
  return new L.Marker.ArrowCircle(options.latLng, options)
}
