...
import { hex2rgbValue } from '../../../utils/pipe/color'
...
export default function (deviceObj, options) {
  ...
  var Renderer = L.Canvas.extend({
    ...
    _updateTransform: function (center, zoom) {
      ...
      var scale = this._map.getZoomScale(zoom, this._zoom),
        ...
        viewHalf = this._map.getSize().multiplyBy(0.5 + options.padding),
    ...
    _updateSector: function (layer) {
      ...
      // 线性渐变
      const rgb = hex2rgbValue(layer.options.color)
      gradient.addColorStop(0, `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0.9)`)
      gradient.addColorStop(1, `rgba(${rgb[0]}, ${rgb[1]}, ${rgb[2]}, 0)`)
      layer.options.fillColor = layer.options.isFill ? layer.options.color : gradient
  ...
  var Radar = L.LayerGroup.extend({
    ...
    onRemove() {
      if (this.RadarArea && this.RadarArea._renderer) {
        this.RadarArea._renderer._containerText && this.RadarArea._renderer._containerText.remove()
        this.RadarArea._renderer.remove && this.RadarArea._renderer.remove()
        this.RadarArea.remove && this.RadarArea.remove()
      }
    },
    getStyle() {
    ...
    update: function (updevice) {
      if (this.deviceObj && updevice) {
        ...
        if (updevice.config && updevice.config.online && updevice.config.online.color) {
          this.deviceObj.config.online.color = updevice.config.online.color
        }
      }
      this.reshArea()
    },
