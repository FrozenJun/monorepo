 ...
 * 返回的是弧度
 */
export function computeAzimuthAndElevation(
  pointA: [number, number, number],
  pointB: [number, number, number]
) {
  // 将经纬高度转换为Cartesian3坐标
  const cartesianA = positionToCartesian(pointA)
  const cartesianB = positionToCartesian(pointB)

  // 计算方向向量
  const direction = Cesium.Cartesian3.subtract(cartesianB, cartesianA, new Cesium.Cartesian3())

  // 计算方位角（azimuth）和俯仰角（elevation）
  const horizontalDistance = Math.sqrt(direction.x * direction.x + direction.y * direction.y)
  const azimuth = Cesium.Math.toDegrees(Math.atan2(direction.y, direction.x))
  const elevation = Cesium.Math.toDegrees(Math.atan2(direction.z, horizontalDistance))
 ...
 * 已知两点经纬高计算距离,米
 */
export function computeCesiumPointsDistance(
  pointA: [number, number, number],
  pointB: [number, number, number]
) {
  // 将经纬高度转换为Cartesian3坐标
...
export function positionToCartesian(position: [number, number, number]) {
