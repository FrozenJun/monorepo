export function isInsidePolygon(point: any, polygonPoints: any[]) {
  const x = point.lng
  const y = point.lat
...
export function isInsideCircle(point: any, center: any, r: number) {
  if (r === 0) return false
  // 将经纬度转换为Cartesian3坐标
  const pointCartesian = Cesium.Cartesian3.fromDegrees(point.lng, point.lat)
  const centerCartesian = Cesium.Cartesian3.fromDegrees(center.lng, center.lat)
