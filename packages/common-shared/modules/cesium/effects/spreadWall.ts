...
class SpreadWall {
  ...
  change_color(val: string) {
    ...
    curEntity._wall._material.color = new Cesium.Color.fromCssColorString(val)
  }
  ...
  add(
    ...
    position = Cesium.Cartographic.fromDegrees(position[0], position[1], position[2])
    ...
    this.viewer.entities.add({
      ...
      wall: {
        ...
        minimumHeights: new Cesium.CallbackProperty(function (n: any) {
  ...
  getmaximumHeights(positions: any) {
    return new Array(positions.length).fill(this.position.height + this.wallHeight)
  }
  getPositions(
    currentRadius: number,
    radius: number,
    ...
    if (
      ((currentRadius += (1000 / duration) * 20), currentRadius > radius && (currentRadius = 1)) // rPositions[currentRadius]
    ) {
